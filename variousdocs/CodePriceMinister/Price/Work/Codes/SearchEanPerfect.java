	#if($line.get("ref_ean") && ($line.get("ref_ean").length() > 12))
	 #set($ean = $line.get("ref_ean").replaceAll("[^0-9A-Za-z]",""))
	 #if(!$id.searchIsbn($ean) && $id.checkEan($ean))
	  $!ean
	 #end
	#elseif($line.get("ref_isbn") && $line.get("ref_isbn").length() > 6)
	 #if($line.get("src_date") && ($tr.toint($tr.extract($line.get("src_date"),"[0-9]{4}")) < 2007))
	  #set($findean = true)
	 #end
	#end
	...
	#if($findean)
	 #* ***********************************
		le partenaire à renseigner un isbn 
		mais pas d'ean ? on peut le déduire
